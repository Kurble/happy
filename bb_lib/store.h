...
namespace bb
{
	...
	public:
		...
		store(const std::string &fromFile);
		...
		int                     getFieldI(const std::string &key) const;
		...
		void                    setFieldI(const std::string &key, const int &val);
	...
	struct store::val
	{
		...
		val(const int &i) : type(Int), i(i), f((float)i) { }
